...
def state_transition(state, action):
    ...
    if action =="plug_in_toaster":
        ...
        # move bread into toaster. Only possible if toaster is not on (cause it locks)
        if not newState["toaster_is_on"]:
            newState["bread_location"] = "toaster"
        newState["time"] += 1
    elif action == "take_out_bread":
        # move bread from toaster to plate. Only possible if toaster is not on (cause it locks)
        if not newState["toaster_is_on"]:
...
def plan(start_state):
    ...
    # the elements are pairs of states and sequences of actions 
    to_visit = [ ([], start_state) ]
    
    # as long as we have new states to visit, visit them
    while len(to_visit) > 0:
        plan, state = to_visit.pop((0))

        # if we have reached the goal, we can return
        if goal(state) == True:
            return plan
        
        # if not, we add the next states to the queue
        for action in actions:
            # checks if action is already in plan
            if action not in plan:
                # Alpha-Beta Pruning
                if action == "plug_in_toaster" and state["toaster_has_power"] == False:
                    new_state = (plan + [action]), state_transition(state, action)
                    to_visit = to_visit + [new_state]

                if action == "unplug_toaster" and state["toaster_is_on"] == True:
                    new_state = (plan + [action]), state_transition(state, action)
                    to_visit = to_visit + [new_state]

                if action == "put_in_bread" and state["bread_location"] == "plate" and state["toaster_is_on"] == False:
                    new_state = (plan + [action]), state_transition(state, action)
                    to_visit = to_visit + [new_state]

                if action == "take_out_bread" and state["bread_location"] == "toaster" and state["toaster_is_on"] == False:
                    new_state = (plan + [action]), state_transition(state, action)
                    to_visit = to_visit + [new_state]

                if action == "switch_on_toaster" and state["toaster_has_power"] == True:
                    new_state = (plan + [action]), state_transition(state, action)
                    to_visit = to_visit + [new_state]

                if action == "wait" and state["bread_location"] == "toaster":
                    new_state = (plan + [action]), state_transition(state, action)
                    to_visit = to_visit + [new_state]
    
    # at this point we exhausted our search options and have not found a goal
    return None
